...
def calculateBodyPartPosVectors(varsPerDataType,noDataTypes,sharedMemoryName,frameLength = 1000,sim = False,idxesToPlot = None):
    ...
    # open shared memory to read live data 
    dataEntries = varsPerDataType * noDataTypes
    SHARED_MEM_NAME = sharedMemoryName
    shared_block_live = shared_memory.SharedMemory(size= dataEntries * 8, name=SHARED_MEM_NAME, create=False)
    shared_array_live = np.ndarray(shape=(noDataTypes,varsPerDataType), dtype=np.float64, buffer=shared_block_live.buf)
    
    # create new shared memory for exporting body vectors
    BODY_PART_MEM = 'Body Parts'
    noCoordinates = 6 # 3 for pos and 3 for vector pos
    noBodyParts = 51
    dataPoints = noCoordinates * noBodyParts
    shared_block_processed = shared_memory.SharedMemory(size= dataPoints * 8, name=BODY_PART_MEM, create=True)
    shared_array_processed = np.ndarray(shape=(noBodyParts,noCoordinates), dtype=np.float64, buffer=shared_block_processed.buf)
    while True:

       
        df = pd.DataFrame(shared_array_live[idxesToPlot])
        df_locations = [df.iloc[a,:] for a in range(0,df.shape[0])] # split rows into list
        df_locations_quaternionObjs = [quaternions.quaternionVector(loc = [a.iloc[4]/1000,a.iloc[5]/1000,a.iloc[6]/1000],quaternion=[a.iloc[0],a.iloc[1],a.iloc[2],a.iloc[3]]) for a in df_locations]
        df_directions =  pd.DataFrame([q.qv_mult(q.quaternion,quaternionsUnit[i]) for i,q in enumerate(df_locations_quaternionObjs)])
        dfPlot = pd.DataFrame({'x':df.iloc[:,4],'y':df.iloc[:,5],'z':df.iloc[:,6],'dirX':df_directions.iloc[:,0],'dirY':df_directions.iloc[:,1],'dirZ':df_directions.iloc[:,2]})
        ...
        minX, minY, minZ = min(df.iloc[:,4].min(),minX), min(df.iloc[:,5].min(),minY), min(df.iloc[:,6].min(),minZ) 
        counter = 0
        numberOfChosenBodyParts = len(quaternionsUnit)
        while counter < numberOfChosenBodyParts:
            shared_array_processed[counter] = dfPlot.iloc[counter,:] 
            counter += 1
        
        print(shared_array_processed)
